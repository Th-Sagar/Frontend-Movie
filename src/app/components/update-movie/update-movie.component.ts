...
export class UpdateMovieComponent {
  ...
  movieCast = new FormControl<string>(
    this.data.movie.movieCast.join(', '),
    Validators.required
  );
  releaseYear = new FormControl<string>(
    this.data.movie.releaseYear.toString(),
    Validators.required
  );
    ...
    if (this.authService.isAuthenticated() && this.updateMovieForm.valid) {
      ...
      const movieDto: MovieDto = {
        ...
        poster: this.selectedFile
          ? this.updateMovieForm.get('poster')?.value
          : null,
      };

      this.movieService
        .updateMovieService(this.movieId, movieDto, this.selectedFile)
        .subscribe({
          next: (response) => {
